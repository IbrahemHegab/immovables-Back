...
exports.createCart = expressAsyncHandler(async (req, res, next) => {
  const galleyModal = await createGallerysModel.findById(req.body.product);
  let cart = await createCartModel.findOne({ user: req.user._id });

  if (!galleyModal) next(new ApiError(`Product ID Not Found`, 404));

  const { product, color } = req.body;
  if (!cart) {
    cart = await createCartModel.create({
      user: req.user._id,
      cartItems: [{ product, color, price: galleyModal.price }],
    });
  } else {
    ...
    if (productExists > -1) {
      ...
      await cart.cartItems.push({ product, price: galleyModal.price });
    }
  }
...
exports.updateSpecificCartItemQuantity = expressAsyncHandler(
  async (req, res, next) => {
    ...
    if (itemsIndex > -1) {
      ...
      // await cart.cartItems.push({ product, color, price: galleyModal.price });
      return next(new ApiError("There is no cart with id "));
    }
    calcTotalPrice(cart);
    await cart.save();
