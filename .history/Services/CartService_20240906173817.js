...
exports.createCart = expressAsyncHandler(async (req, res, next) => {
  const galle = await createGallerysModel.findById(req.body.product);
  let cart = await createCartModel.findOne({ user: req.user._id });

  if (!galle) next(new ApiError(`Product ID Not Found`, 404));

  const { product, color } = req.body;
  if (!cart) {
    cart = await createCartModel.create({
      user: req.user._id,
      cartItems: [{ product, color, price: galle.price }],
    });
  } else {
    ...
    if (productExists > -1) {
      ...
      await cart.cartItems.push({ product, color, price: galle.price });
    }
  }
