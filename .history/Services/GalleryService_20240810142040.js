...
if (req.files.images) {
  ...
  await Promise.all(
    req.files.images.map(async (ele, inx) => {
      const imageGallery = `imageGallery-${uuidv4()}-${Date.now()}-${inx + 1}.jpeg`;
      ...
      req.body.images.push({
        filename: imageGallery,
        image: `/uploads/gallery/${imageGallery}`, // يمكنك تعديل المسار بناءً على ما تحتاجه
        // يمكنك إضافة أي معلومات أخرى تحتاجها هنا
      });
    })
  );
