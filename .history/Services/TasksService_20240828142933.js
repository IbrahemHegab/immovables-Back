...
exports.createTask = async (req, res) => {
  ...
  try {
    ...
    const populatedTask = await TaskModel.findById(task._id).populate("assignedBy.name");

    const user = await createEmployeesModel.findById(assignedTo);
    if (user) {
      user.notifications.push({
        assignedBy: populatedTask.assignedBy, // هنا يتم إضافة معلومات الشخص الذي قام بتعيين المهمة
      });
