    ...
    expressAsyncHandler(async (req, res, next) => {
      ...
      try {
        ...
        if (req.body.images && typeof req.body.images === "object") {
          for (const key in req.body.images) {
            if (req.body.images.hasOwnProperty(key)) {
              if (findDocument.images && findDocument.images[key] && findDocument.images[key] !== req.body.images[key]) {
                const relativePathImage = findDocument.images[key].split(baseUrl)[1];
        ...
        if (req.body.images && typeof req.body.images === "object") {
          for (const key in req.body.images) {
            if (req.body.images.hasOwnProperty(key)) {
              updateData[`images.${key}`] = req.body.images[key];
            }
          }
        }
