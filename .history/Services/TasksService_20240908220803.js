...
exports.completeTask = expressAsyncHandler(async (req, res, next) => {
  const populatedTask = await TaskModel.findById(req.params.id).populate({
    path: "assignedBy",
  });

  const user = await createEmployeesModel.findById(
    populatedTask.assignedBy._id
  );
  if (user) {
    user.notifications.push({
...
exports.rejectedTask = expressAsyncHandler(async (req, res, next) => {
  ...
  //   await user.save();
  // }
  const populatedTask = await TaskModel.findById(._id).populate({
    path: "assignedBy",
  });
  const user = await createEmployeesModel.findById(populatedTask.assignedBy._id);
  if (user) {
    user.notifications.push({
      assignedBy: populatedTask.assignedBy.name,
      task: task._id, // هنا يتم إضافة معلومات الشخص الذي قام بتعيين المهمة
