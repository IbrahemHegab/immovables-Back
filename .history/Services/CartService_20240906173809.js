...
exports.createCart = expressAsyncHandler(async (req, res, next) => {
  const galley = await createGallerysModel.findById(req.body.product);
  let cart = await createCartModel.findOne({ user: req.user._id });

  if (!galley) next(new ApiError(`Product ID Not Found`, 404));

  const { product, color } = req.body;
  if (!cart) {
    cart = await createCartModel.create({
      user: req.user._id,
      cartItems: [{ product, color, price: galley.price }],
    });
  } else {
    ...
    if (productExists > -1) {
      ...
      await cart.cartItems.push({ product, color, price: galley.price });
    }
  }
